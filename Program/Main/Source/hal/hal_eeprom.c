...
static BOOL_T	ByteWrite( U8 _dev, U16 _addr , U8 _data )
{
    ...
    if( i2ca0_master_send( _dev | EEP_OP_WR , &buf[1] , 2 , 200 ) != MD_OK )
    {
        CountWriteError();
...
static BOOL_T	PageWrite( U8 _dev, U16 _addr , U8 *_data, U8 _len )
{
    ...
    memcpy( &buf[ 2 ], &_data[ 0 ], _len );

    HAL_RTC_SetWriteProc( EEP_PROC_START );

    // Write Page..
    //if( i2ca0_master_send( _dev | EEP_OP_WR , &buf[ 0 ] , _len + 2 , 200 ) != MD_OK )
    if( i2ca0_master_send( _dev | EEP_OP_WR , &buf[ 1 ] , _len + 1 , 200 ) != MD_OK )
    {
        CountWriteError();
        i2ca0_StopCondition();
        return FALSE;
...
static BOOL_T	ByteRead( U8 _dev, U16 _addr , U8 *_data )
{
    ...
    if( i2ca0_master_send( _dev | EEP_OP_WR , &buf[1] , 1 , 200 ) != MD_OK )
    {
        CountReadError();
        i2ca0_StopCondition();
        return FALSE;
...
BOOL_T	HAL_RTC_EEPROM_AllErase( void )
{
    ...
    memset( &buf[ 2 ], 0x00, EEP_PAGE_SIZE );
    ...
    for( i = 0; i < EEP_PAGE_NUM; i++ )
    {
        ...
        if( i2ca0_master_send( DEV_ADDR_EEP | EEP_OP_WR , &buf[ 1 ] , EEP_PAGE_SIZE + 1 , 200 ) != MD_OK )
        {
            CountWriteError();
            i2ca0_StopCondition();
            return FALSE;
