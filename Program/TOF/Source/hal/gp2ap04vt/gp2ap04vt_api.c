...
void gp2ap04vt_init_sensor(struct gp2ap04vt_sensor *pdev)
{
    ...
    gp2ap_i2c_write(pdev, 0x22, wdata_multi, 4);
    // conv_time_set1_user
    wdata_multi[0] = SET_CONV2;
    gp2ap_i2c_write(pdev, 0x22, wdata_multi, 4);

    // address
    wdata = 0x40; gp2ap_i2c_write(pdev, 0x20, &wdata, 1);
    wdata = 0x01; gp2ap_i2c_write(pdev, 0x21, &wdata, 1);
    ...
    // address
    wdata = 0x2C; gp2ap_i2c_write(pdev, 0x20, &wdata, 1);
    wdata = 0x02; gp2ap_i2c_write(pdev, 0x21, &wdata, 1);
    // th_prox 
    wdata_multi[0] = SET_TH_PROX;
    gp2ap_i2c_write(pdev, 0x22, wdata_multi, 4);
    // th_range_l 
    wdata_multi[0] = SET_TH_RANGE_L;
    gp2ap_i2c_write(pdev, 0x22, wdata_multi, 4);
    // th_range_h 
    wdata_multi[0] = SET_TH_RANGE_H;
    gp2ap_i2c_write(pdev, 0x22, wdata_multi, 4);
    // th_hys 
    ...
    if (pdata->mode == MODE_OFFSET_CALIBRATION) {
        wdata_multi[0] = OFFSET_CALIBRAION_DISTANCE_MM; // The distance spec is 10cm(100mm). 
    } else {
        wdata_multi[0] = 0x00;
    }
    gp2ap_i2c_write(pdev, 0x22, wdata_multi, 4);
    ...
    wdata = 0x04; gp2ap_i2c_write(pdev, 0x1E, &wdata, 1);
